...
alter table agencies add check (phone_number like '+996%');
...
select houses.house_type,houses.price,houses.description,houses.room,houses.furniture,a.city,a.region,a.street from houses join addresses a on houses.address_id = a.id where a.id in (5,6,7,8,9,10);
...
select * from customers  where date_of_birth between date '1999-01-01' and date '2023-01-01'   limit 15 offset 10;
...
select house_type,sum(price),count(*) from houses  where house_type = 'Apartment' group by house_type;
